...
async def check_and_create_qdrant_points():
    ...
    op.add_column(
        "textchunk",
        sa.Column("sparse_embedding", SPARSEVEC, nullable=True),
    )
...
async def create_missing_points(
    ...
    for chunk in missing_chunks:
        try:
            # No need to regenerate dense vector as it already overlaps from the chunking

            # Regenerate sparse vector for chunks without it
            ...
            chunk.sparse_embedding = sparse_str
...
def downgrade() -> None:
    ...
    op.drop_column("textchunk", "sparse_embedding")
